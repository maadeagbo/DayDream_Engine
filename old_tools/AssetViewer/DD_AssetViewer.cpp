...
void createNewModelSk(DD_Resources* res_ptr, const char* id, const char* path,
                      const char* skeleton_id);
void createNewAgentSk(DD_Resources* res_ptr, const char* agent_id,
                      const char* mdlsk_id);
void createNewAnimation(DD_Resources* res_ptr, const char* id,
                        const char* path);

enum class ImportType : unsigned { FBX, DDM, DDM_S, DDM_S_A, NONE };
  ...
  if (event.m_type == "new_mdlsk") {
    ...
    if (_m) {
      dd_array<cbuff<256>> args =
          StrSpace::tokenize512<256>(_m->message512, " ");
      if (args.size() >= 3) {
        createNewModelSk(res_ptr, args[0].str(), args[1].str(), args[2].str());
      } else {
        DD_Terminal::post(
            "new_mdlsk <model id> <model path> "
            "<skeleton id>");
      }
    }
  }
...
void createNewSkeleton(DD_Resources* res_ptr, const char* id,
                       const char* path) {
...
void createNewAgentSk(DD_Resources* res_ptr, const char* agent_id,
  ...
  if (!mdl) {
    DD_Terminal::f_post(
        "[error] DD_ModelSk <%s> not found. Aborting "
        "agent creation.",
        mdlsk_id);
    return;
  }
  ...
  if (agent) {
    DD_Terminal::f_post(
        "[error] DD_Agent <%s> already exists. Aborting "
        "agent creation.",
        agent_id);
    return;
  }
