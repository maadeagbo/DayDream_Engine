  ...
  if (event.m_type == "add_anim") {
    ...
    if (_m) {
      ...
      if (args.size() >= 3) {
        ...
        if (same) {
          appendAnimation(args[1].str(), args[2].str());
        }
      } else {
        DD_Terminal::post(
            "add_anim <agent id> "
...
void AssetViewAvatar::appendAnimation(const char* animation_id,
  ...
  if (!skinnedMdl) {
    DD_Terminal::f_post(
        "[error] DD_ModelSk <%s> could not be found. "
        "Aborting animation append.",
        animation_id);
    return;
  }

  unsigned idx = (unsigned)skinnedMdl->m_animStates.size();
  bool added = ResSpace::addAnimationToModel(res_ptr, skinnedMdl->m_ID.c_str(),
                                             animation_id, new_ref_id);
