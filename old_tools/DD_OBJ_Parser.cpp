...
ObjAssetParser::ParseFlag ObjAssetParser::PreProcess(ObjAsset& obj,
  ...
  if (file.good()) {
    ...
    std::strcpy(obj.file_buffer, tempStr.c_str());
    file.close();
    std::string line;
    ...
    while (std::getline(analysisBuf, line)) {
      ...
      if (lineID == "usemtl") {
        ...
        obj.info.mesh_indices[obj.info.num_meshes].start = obj.info.num_f;
    ...
    obj.info.mesh_indices[obj.info.num_meshes - 1].end = obj.info.num_f - 1;
...
void ObjAssetParser::PrintInfo(ObjAsset& obj) {
  ...
  for (size_t i = 0; i < obj.info.mesh_indices.size(); i++) {
    printf("Mesh %zd: \n\tstart: %zd \n\tend: %zd \n\ttriangles: %zd\n", i,
           obj.info.mesh_indices[i].start, obj.info.mesh_indices[i].end,
           obj.info.mesh_indices[i].tris);
  }
  for (size_t i = 0; i < obj.info.mat_buffer.size(); i++) {
    printf("\nMaterial %zd: \n\tID: %s\n", i,
           obj.info.mat_buffer[i].ID.c_str());
    ...
    while (std::getline(analysisBuf, line)) {
      ...
      if (lineID == "v") {
        ...
        for (size_t i = 0; i < triangles.num_tris; i++) {
          obj.meshes[mesh_index].data[element_index] = triangles.verts[i * 3];
          obj.meshes[mesh_index].indices[element_index] = (int)element_index;
          element_index += 1;
        ...
        // get mat info
        for (size_t i = 0; i < obj.info.mat_buffer.size(); i++) {
          bool test = obj.info.mat_IDs[mesh_index] == obj.info.mat_buffer[i].ID;
          if (test) {
            obj.meshes[mesh_index].material_info = obj.info.mat_buffer[i];
            // get mat ID
...
void ObjAssetParser::CreateDDMesh(ObjAsset& obj) {
  ...
  for (size_t i = 0; i < obj.meshes.size(); i++) {
    ...
    outfile << "mat_dif " << mat.diffuseRaw.x() << " " << mat.diffuseRaw.y()
            << " " << mat.diffuseRaw.z() << "\n";
    ...
    for (size_t j = 0; j < m.data.size(); j++) {
      ...
      outfile << "pos " << vert.position[0] << " " << vert.position[1] << " "
              << vert.position[2] << " "
              << "\n";
      outfile << "norm " << vert.normal[0] << " " << vert.normal[1] << " "
              << vert.normal[2] << " "
              << "\n";
      outfile << "tan " << vert.tangent[0] << " " << vert.tangent[1] << " "
              << vert.tangent[2] << " "
              << "\n";
  ...
  if (inFile.good()) {
    ...
    while (std::getline(analysisBuf, line)) {
      ...
      if (lineID == "NUM_MESH") {
        ...
        obj.meshes[meshIndex].material_info.directory = obj.info.directory;
...
void ObjAssetParser::CalculateTangentSpace(ObjAsset& obj,
                                           const size_t meshIndex) {
...
#pragma omp parallel for
  for (int i = 0; i < (int)range; i += 3) {
    ...
    uv1 = obj_vec3(vertices[indices[i]].texCoords[0],
                   vertices[indices[i]].texCoords[1], 0.0f);
    uv2 = obj_vec3(vertices[indices[i + 1]].texCoords[0],
                   vertices[indices[i + 1]].texCoords[1], 0.0f);
    uv3 = obj_vec3(vertices[indices[i + 2]].texCoords[0],
                   vertices[indices[i + 2]].texCoords[1], 0.0f);
    edge1 = vert2 - vert1;
    edge2 = vert3 - vert1;
    deltaUV1 = uv2 - uv1;
    deltaUV2 = uv3 - uv1;
...
FaceVerts SetMultiMeshFaceData(const std::string& data, ObjAsset& obj,
  ...
  for (int i = 0; i < (int)result.num_tris; ++i) {
    ...
    std::string trisStr[] = {face_verts[0], face_verts[i + 1],
                             face_verts[i + 2]};
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      Vertex vert =
          GetVertexFromFaceInfo(trisStr[j], obj.info.f_style[meshIndex], obj);
