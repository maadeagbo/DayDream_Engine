...
enum AI_ALGORITHMS { A_STAR, DIJKSTRA, NUM_ALGOS };
...
struct AI_Agent {
  ...
  bool plan_now = false, calc_heuristics = true, move_now = false,
       replan = false;
  dd_array<float> heuristics;
  dd_array<glm::vec3> path;
  heuristic_func h_func;
  inline void init(const glm::vec3 start, const glm::vec3 end,
                   const float _radius, const float speed,
                   const char* ai_object_ID, heuristic_func func_pointer) {
...
struct DD_AIObject {
  ...
  inline void init(const AI_ALGORITHMS algo, const glm::vec3 ground_pos,
                   const glm::vec3 ground_max_corner,
                   const glm::vec3 ground_min_corner,
                   const size_t total_nodes = 10,
                   const size_t lines_per_node = 5,
                   const float max_agent_radius = 0,
                   const char* node_mesh_ID = "",
