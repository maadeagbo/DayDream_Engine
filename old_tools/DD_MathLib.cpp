...
void DD_MathLib::setResourceBin(DD_Resources* res_bin) { res = res_bin; }
...
bool DD_MathLib::checkRayCircle(const glm::vec3 ray, const glm::vec3 point,
  ...
  if ((glm::dot(-norm_ray, center - end) < 0.f) &&
      glm::length(center - end) > r) {
...
// If agentID is empty, check all object in scene (can be optimized w/ spatial
// DS)
raycastBuff DD_MathLib::rayCast(const int mouseX, const int mouseY,
                                const char* agentID) {
  raycastBuff rcbuff;
  rcbuff.hit = false;
  // calculate screen space to world space point (normalize ray direction from
  // camera)
  DD_Camera* cam;
...
size_t DD_MathLib::calculateLOSLines(const size_t index,
  ...
  for (size_t i = 0; i < other.size() && count < container.size(); i++) {
    ...
    for (size_t j = 0; j < obst.size(); j++) {
      ...
      if (obst[j].type == 0) {
        // circles
        hit[j] = checkRayCircle(ray, point, pos, obst[j].radius + agent_radius);
      } else {
        // boxes
      }
    }
