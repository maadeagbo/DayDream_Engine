...
bool DD_Material::OpenGLBindMaterial()
{
	if (!m_loaded_to_GPU) {
		...
		if (m_albedo) {
			success = BindTexture(TextureType::ALBEDO, 
								  m_textures[TextureType::ALBEDO]);
			if (!success) { return false; }
		}
		if (m_specular) {
			success = BindTexture(TextureType::SPECULAR, 
								  m_textures[TextureType::SPECULAR]);
			if (!success) { return false; }
		}
		if (m_normal) {
			success = BindTexture(TextureType::NORMAL,
								  m_textures[TextureType::NORMAL]);
			if (!success) { return false; }
		}
		if (m_roughness) {
			success = BindTexture(TextureType::ROUGH, 
								  m_textures[TextureType::ROUGH]);
			if (!success) { return false; }
		}
		if (m_metalness) {
			success = BindTexture(TextureType::METAL, 
								  m_textures[TextureType::METAL]);
			if (!success) { return false; }
		}
		if (m_ambient) {
			success = BindTexture(TextureType::AO, 
								  m_textures[TextureType::AO]);
			if (!success) { return false; }
		}
		if (m_emissive) {
			success = BindTexture(TextureType::EMISSIVE, 
								  m_textures[TextureType::EMISSIVE]);
			if (!success) { return false; }
		}
	}
