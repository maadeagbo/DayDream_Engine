...
void BoundingBox::SetCorners()
{
    ...
    this->corner1 = glm::vec3(this->min.x, this->min.y, this->min.z); // back bottom left
    this->corner2 = glm::vec3(this->max.x, this->min.y, this->min.z); // back bottom right
    this->corner3 = glm::vec3(this->max.x, this->max.y, this->min.z); // back top right
    this->corner4 = glm::vec3(this->min.x, this->max.y, this->min.z); // back top left
    this->corner5 = glm::vec3(this->min.x, this->min.y, this->max.z); // front bottom left
    this->corner6 = glm::vec3(this->max.x, this->min.y, this->max.z); // front bottom right
    this->corner7 = glm::vec3(this->max.x, this->max.y, this->max.z); // front top right
    this->corner8 = glm::vec3(this->min.x, this->max.y, this->max.z); // front top left
}
...
glm::vec3 BoundingBox::UpdateAABB_min()
{
    ...
    glm::vec3 points[] = { this->corner2, this->corner3, this->corner4,
        this->corner5, this->corner6, this->corner7, this->corner8 };
...
glm::vec4 getVec4f(const char* str)
{
	glm::vec4 v4;
	char* nxt;
	v4[0] = strtof(str, &nxt); nxt++;
	v4[1] = strtof(nxt, &nxt); nxt++;
	v4[2] = strtof(nxt, &nxt); nxt++;
...
glm::uvec4 getVec4u(const char* str)
{
	glm::uvec4 v4;
	char* nxt;
	v4[0] = strtoul(str, &nxt, 10); nxt++;
	v4[1] = strtoul(nxt, &nxt, 10); nxt++;
	v4[2] = strtoul(nxt, &nxt, 10); nxt++;
...
glm::quat getQuat(const char* str)
{
    char* nxt;
    float x, y, z;
    x = strtof(str, &nxt); nxt++;
    y = strtof(nxt, &nxt); nxt++;
...
glm::mat4 createMatrix(const glm::vec3& pos,
					   const glm::vec3& rot,
					   const glm::vec3& scale)
{
	glm::mat4 mt = glm::translate(glm::mat4(), pos);
	glm::mat4 mrx = glm::rotate(glm::mat4(), 
								glm::radians(rot.x), 
								glm::vec3(1.f, 0.f, 0.f));
	glm::mat4 mry = glm::rotate(glm::mat4(), 
								glm::radians(rot.y), 
								glm::vec3(0.f, 1.f, 0.f));
	glm::mat4 mrz = glm::rotate(glm::mat4(), 
								glm::radians(rot.z), 
