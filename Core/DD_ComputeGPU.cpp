...
*/
DD_Compute::~DD_Compute() { glDeleteBuffers(2, m_kinectDepthBuff); }

/**
        Initializes any neccessary buffers from computing
        \todo Need to cannabalize this section and lock buffers to cumpute
   requests
*/
...
DD_Event DD_Compute::Compute(DD_Event& event) {
  ...
  if (event.m_type == "compute_texA") {
    ...
    DD_Texture2D* tex =
        ResSpace::findDD_Texture2D(res_ptr, _ct->texture_ID.c_str());
    DD_Shader* shader =
        ResSpace::findDD_Shader(res_ptr, _ct->shader_ID.c_str());
    int numX = 512 / (int)_ct->data_info[0];
    int numY = 424 / (int)_ct->data_info[1];

    bool second_buffer = false;
    glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_kinectDepthBuff[0]);
    glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, (GLsizeiptr)_ct->byte_size01,
                    _ct->data_01);
    if (_ct->byte_size02 > 0 && _ct->data_02) {
      second_buffer = true;
    }
    ...
    if (second_buffer) {
      glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_kinectDepthBuff[1]);
      glBufferSubData(GL_SHADER_STORAGE_BUFFER, 0, (GLsizeiptr)_ct->byte_size02,
                      _ct->data_02);
    }
    ...
    if (_ct->texture2_ID != "") {
      DD_Texture2D* tex2 =
          ResSpace::findDD_Texture2D(res_ptr, _ct->texture2_ID.c_str());
    ...
    glGetBufferSubData(GL_SHADER_STORAGE_BUFFER, 0,
                       (GLsizeiptr)_ct->byte_size01, _ct->data_01);
    if (second_buffer) {
      glBindBuffer(GL_SHADER_STORAGE_BUFFER, m_kinectDepthBuff[1]);
      glGetBufferSubData(GL_SHADER_STORAGE_BUFFER, 0,
                         (GLsizeiptr)_ct->byte_size02, _ct->data_02);
    }
