...
void localToGlobalPose(DD_SkeletonPose& pose,
...
void DD_AnimSystem::processAnimState(DD_ModelSK* mdlsk,
									 const DD_Skeleton* sk,
									 const float time)
{
	...
	for (unsigned i = 0; i < mdlsk->m_animStates.size(); i++) {
		...
		if (!a_state.active) { continue; } // skip inactive animations

		DD_AnimClip* a_clip = ResSpace::findDD_AnimClip(
			res_ptr, a_state.clip_id.str());
		dd_array<DD_JointPose>& localp = mdlsk->m_finalPose.m_localPose;

		// calculate time
		float anim_speed = a_state.play_back;
		if (a_state.pause) { anim_speed = 0.f; } // pause 
		...
		const unsigned idx_a =
			(unsigned)(a_state.local_time / a_clip->step_size);
		// clamp interpolating frame
		const unsigned idx_b = (idx_a + 1) < a_clip->num_frames ? 
			(idx_a + 1) : a_clip->num_frames - 1;
		const float alpha = (a_state.local_time-(idx_a * a_clip->step_size))/
			a_clip->step_size;
					   ...
					   bool globalflag)
{
	if (!visited[idx]) {
		...
		if (idx == 0) {
			...
			if (!globalflag) {
				pose.m_globalPose[idx] = pose.m_globalPose[p_idx] *
					sk->m_bones[idx].m_pDelta * trans * rot;
			}
			else {
