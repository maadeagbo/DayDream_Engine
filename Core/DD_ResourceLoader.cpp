...
DD_Texture2D* CreateTexture_OBJMAT(DD_Resources* res,
								   std::string dir,
								   std::string tex_path);
		...
		while (std::getline(analysisBuf, line)) {
			...
			if (lineID == "<Agent>") {
				...
				for (size_t i = 0; i < num_inst; i++) {
					...
					// create new model
					agent->AddModel(
						tempStr[0].c_str(), near_far.x(), near_far.y());
					agent->UpdatePosition(glm::vec3(_p.x(), _p.y(), _p.z()));
					agent->UpdateRotation(
						glm::quat(glm::vec3(_r.x(), _r.y(), _r.z())));
			...
			if (lineID == "<Model>") {
				...
				const std::string mtlname = (tempStr[1] == "<>") ?
					"" : tempStr[1];
			...
			if (lineID == "<Light>") {
				...
				// Linear
				if (tempStr[5] != "<>") {
					res->lights[res->light_counter - 1].m_linear = float(
						std::atof(tempStr[5].c_str()));
				}
				// Quadratic
				if (tempStr[6] != "<>") {
					res->lights[res->light_counter - 1].m_quadratic = float(
						std::atof(tempStr[6].c_str()));
				}
			}
			if (lineID == "<Camera>") {
				res->cam_counter += 1;
				...
				// Far
				if (tempStr[1] != "<>") {
					res->cameras[res->cam_counter - 1].far_plane = float(
						std::atof(tempStr[1].c_str()));
				}
				// Near
				if (tempStr[2] != "<>") {
					res->cameras[res->cam_counter - 1].near_plane = float(
						std::atof(tempStr[2].c_str()));
				}
				// FOV H
				if (tempStr[3] != "<>") {
					res->cameras[res->cam_counter - 1].fov_h = float(
						std::atof(tempStr[3].c_str()));
				}
				// Position and rotation
				glm::vec4 pos = glm::vec4(
					cam_pos.x(), cam_pos.y(), cam_pos.z(), 1.0f
				);
				glm::quat cam_q = glm::quat(
					glm::vec3(cam_rot.x(), cam_rot.y(), cam_rot.z())
				);
...
DD_Material createMaterial(DD_Resources* res, obj_mat& matbuff)
{
	...
	mat.m_ID = matbuff.ID;
	mat.m_base_color = glm::vec4(
		matbuff.diffuseRaw.x(),
		matbuff.diffuseRaw.y(),
		matbuff.diffuseRaw.z(),
		1.0f
	);
...
DD_Model loadModel(DD_Resources* res, 
				   const char * obj_path, 
				   const char * mtlName)
{
	...
	if (parseResult.success && !parseResult.ddMesh) {
		...
		for (size_t i = 0; i < asset.info.mat_buffer.size(); i++) {
			...
			res->materials[res->mtl_counter] = std::move(
				ResSpace::createMaterial(res, _mat));
			res->mtl_counter += 1;
		}
...
DD_ModelSK * loadSkinnedModel(DD_Resources * res,
							  const char * new_mdl_id,
							  ...
							  const char * ddb_path)
{
	...
	if (!sk) {
		if (!loadDDB(res, ddb_path, skeleton_id)) {
			DD_Terminal::f_post(
				"[error] loadSkinnedModel::Invalid ddb file: %s", ddb_path);
			return mdlsk;
		}
	}
...
DD_Model* loadModel_MD(DD_Resources* res,
					   const char* id,
					   const dd_array<MeshData>& md,
					   DD_Model* mdl)
{
	...
	for (size_t i = 0; i < md.size(); i++) {
		...
		res->materials[res->mtl_counter] = std::move(
			ResSpace::createMaterial(res, mat));
		res->mtl_counter += 1;
	}
...
DD_Model * loadModel_DDM(DD_Resources * res,
						 const char * id,
						 const char * ddm_path)
{
	cbuff<512> path(ddm_path);
	...
	if (io_handle.open(path, DD_IOflag::READ)) {
		...
		while (nxtLine) {
			...
			if (mybuff.compare("<global>") == 0) {
				...
				nxtLine = io_handle.readNextLine(); nxtLine += 2; // rotation
				glm::vec3 rot = getVec3f(nxtLine);
				nxtLine = io_handle.readNextLine(); nxtLine += 2; // scale
		...
		for (unsigned i = 0; i < skele->m_bones.size(); i++) {v_bin[i] = false;}
			...
			if (mybuff.compare("<animation>") == 0) {
				...
				while(*nxtLine != '<') { // keep reading till </animation>
					...
					if (*nxtLine == 'p') { // joint translation
						val = nxtLine + 2;
						a_clip->samples[f_idx - 1].m_pose[idx].m_trans =
							getVec3f(val);
						//printf("t %u:%u %s\n", f_idx, idx, val);
					}
					nxtLine = io_handle.readNextLine();
...
void initCameras(DD_Resources * res, const float width, const float height)
{
	for (size_t i = 0; i < res->cam_counter; i++) {
		...
		res->cameras[i].updateCamera(
			res->cameras[i].pos(), res->cameras[i].camQuat()
		);
...
bool addAnimationToModel(DD_Resources* res,
                         const char* model_id,
                         const char* clip_id,
...
bool loadAgentToGPU(DD_Resources * res, const size_t index)
{
	...
	const bool hasModel = res->agents[index]->flag_model ||
		res->agents[index]->flag_modelsk;
	if (!hasModel){
		return true;
	}
	...
	for (size_t i = 0; i < camsInScene; i++) {
		...
		if (cam->isChild()) {
			...
			if (cam->parentIndex() == -1) {
				...
				cam->parent_transform =
					UpdateInstance(cam->parentIndex(), res, dt);
	...
	for (size_t i = 0; i < lghtInScene; i++) {
		...
		if (lght->isChild()) {
			...
			if (lght->parentIndex() == -1) {
				...
				// valid parent index, set transform
				lght->parent_transform =
					UpdateInstance(lght->parentIndex(), res, dt);
	...
	for (size_t i = 0; i < objsInScene; i++) {
		...
		if (agent->isChild()) {
			...
			if (agent->parentIndex() == -1) {
				ResSpace::setAgentParent(res, agent,
										 agent->parentID().c_str());
				// setAgentParent will remove child attachment if parent not found
			}
			else {
				// valid parent index, set transform
				agent->parent_transform =
					UpdateInstance(agent->parentIndex(), res, dt);
			}
		}
		else {
...
void deleteAgent(DD_Resources * res, const char * agent_id, bool free_gpu_memory)
{
								   ...
								   std::string tex_path)
{
	for (size_t i = 0; i < res->tex_counter; i++) {
		...
		if (tex.m_ID == tex_path) { return &tex; }
	}
...
dd_array<MeshData> getMeshData(FbxVertData &vdata, dd_array<FbxEboData>& edata)
{
	...
	{
		// max
		mesh.bbox_max.x() = (mesh.bbox_max.x() > test.x) ?
			mesh.bbox_max.x() : test.x;
		mesh.bbox_max.y() = (mesh.bbox_max.y() > test.y) ?
			mesh.bbox_max.y() : test.y;
		mesh.bbox_max.z() = (mesh.bbox_max.z() > test.z) ?
			mesh.bbox_max.z() : test.z;
		// min
		mesh.bbox_min.x() = (mesh.bbox_min.x() < test.x) ?
			mesh.bbox_min.x() : test.x;
		mesh.bbox_min.y() = (mesh.bbox_min.y() < test.y) ?
			mesh.bbox_min.y() : test.y;
		mesh.bbox_min.z() = (mesh.bbox_min.z() < test.z) ?
			mesh.bbox_min.z() : test.z;
	};
...
void populateFbxEboData(FbxEboData &edata, std::ifstream &_stream)
{
	...
	auto getVec3u = [&](const char* str)
	{
		glm::uvec3 v3;
		char* nxt;
		v3[0] = strtoul(str, &nxt, 10); nxt++;
		v3[1] = strtoul(nxt, &nxt, 10); nxt++;
	...
	while (strcmp("</ebo>", line) != 0) {
		...
		switch (*line) {
			...
			case '-':	// triangle indices
				v3u = getVec3u(&line[2]);
				edata.idxs[idx] = v3u.x; idx++;
				edata.idxs[idx] = v3u.y; idx++;
				edata.idxs[idx] = v3u.z; idx++;
				break;
			default:
				break;
		}
...
glm::mat4 UpdateInstance(const int index, DD_Resources* rsrc, float thisFrame)
{
	...
	if (agent->checkFrame() < thisFrame) {
		...
		if (agent->isChild()) {
			...
			if (agent->parentIndex() == -1) {
				bool parentFound = ResSpace::setAgentParent(
					rsrc, agent, agent->parentID().c_str());

				if (parentFound) {
					// recursive lookup
					return myMat *
						UpdateInstance(agent->parentIndex(), rsrc, thisFrame);
				}
...
void calcDeltaBoneTransform(DD_Skeleton* sk, bool* visited, const unsigned idx)
{
	if (!visited[idx]) {
		...
		// check parent bone
		if (!visited[p_idx]) { calcDeltaBoneTransform(sk, visited, p_idx); }
		if (idx == 0) {
