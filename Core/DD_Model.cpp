...
BoundingBox ModelSpace::CalculateBBox(const DD_Model & model)
{
	...
	for( size_t i = 0; i < model.meshes.size(); i++ ) {
		if( firstMesh ) {
			bbox.min = glm::vec3(
				model.meshes[i].bbox_min.x(),
				model.meshes[i].bbox_min.y(),
				model.meshes[i].bbox_min.z()
			);
			bbox.max = glm::vec3(
				model.meshes[i].bbox_max.x(),
				model.meshes[i].bbox_max.y(),
				model.meshes[i].bbox_max.z()
			);
			firstMesh = false;
		}
		else {
			glm::vec3 vectorMin = glm::vec3(
				model.meshes[i].bbox_min.x(),
				model.meshes[i].bbox_min.y(),
				model.meshes[i].bbox_min.z()
			);
			glm::vec3 vectorMax = glm::vec3(
				model.meshes[i].bbox_max.x(),
				model.meshes[i].bbox_max.y(),
				model.meshes[i].bbox_max.z()
			);
...
void ModelSpace::OpenGLBindMesh(const int index,
								DD_Model & model,
...
void ModelSpace::PrintInfo(const DD_Model & mod)
{
	...
	for( size_t i = 0; i < mod.meshes.size(); i++ ) {
		...
		printf("\tBBox min: %.3f, %.3f, %.3f\n",
			   ...
			   mod.meshes[i].bbox_min.y(),
			   mod.meshes[i].bbox_min.z()
		);
		printf("\tBBox max: %.3f, %.3f, %.3f\n",
			   mod.meshes[i].bbox_max.x(),
			   mod.meshes[i].bbox_max.y(),
			   mod.meshes[i].bbox_max.z()
		);
