...
bool DD_Texture2D::Refill(const char* full_path) {
  path = full_path;
  unsigned char* image = SOIL_load_image(full_path, &(this->Width),
                                         &(this->Height), 0, SOIL_LOAD_RGBA);
...
bool DD_Skybox::Generate() {
  ...
  unsigned char* image = SOIL_load_image((TEX_DIR + right).c_str(), &m_width,
                                         &m_height, 0, SOIL_LOAD_AUTO);
  SOIL_free_image_data(image);
  ...
  for (int i = 0; i < 6; i++) {
    ...
    image =
        SOIL_load_image(file.c_str(), &m_width, &m_height, 0, SOIL_LOAD_RGBA);
    if (image == nullptr) {
