...
void DD_Camera::SetupFrustum()
{
	...
	// D
	cam_frustum.d[0] = -glm::dot(cam_frustum.normals[0], cam_frustum.points[0]);
	cam_frustum.d[1] = -glm::dot(cam_frustum.normals[1], cam_frustum.points[1]);
	cam_frustum.d[2] = -glm::dot(cam_frustum.normals[2], cam_frustum.points[2]);
	cam_frustum.d[3] = -glm::dot(cam_frustum.normals[3], cam_frustum.points[3]);
	cam_frustum.d[4] = -glm::dot(cam_frustum.normals[4], cam_frustum.points[4]);
	cam_frustum.d[5] = -glm::dot(cam_frustum.normals[5], cam_frustum.points[5]);
}
...
glm::mat4 CamSpace::GetViewMatrix(const DD_Camera * cam)
{
	...
	glm::vec3 right = glm::normalize(
		glm::cross(front, glm::vec3(cam->worldUp())));
...
*/
glm::mat4 CamSpace::GetViewMatrixVREye(const DD_Camera * cam,
									   const bool _left,
									   const bool _right,
									   const float dist)
{
...
*/
glm::mat4 CamSpace::GetOffAxisProjMatrix(
	const DD_Camera * cam,
	...
	const glm::vec3 eyePos)
{
	...
	glm::vec3 wcsTopLeftScr(
		screenPos.x - scrWidth / 2,
		screenPos.y + scrHeight / 2,
		screenPos.z);
	glm::vec3 wcsBotRightScr(
		screenPos.x + scrWidth / 2,
		screenPos.y - scrHeight / 2,
		screenPos.z);
	...
	glm::vec3 camTopLeftNear = cam->near_plane / camTopLeftScr.z * camTopLeftScr;
	glm::vec3 camBotRightScr = wcsBotRightScr - eyePos;
	glm::vec3 camBotRightNear = camBotRightScr / camBotRightScr.z * cam->near_plane;
	...
	glm::mat4 frst = glm::frustum(
		-camTopLeftNear.x, -camBotRightNear.x,
		-camBotRightNear.y, -camTopLeftNear.y,
