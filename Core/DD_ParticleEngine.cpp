...
bool DD_ParticleSys::Init() {
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec4) * 500000, NULL,
               GL_DYNAMIC_DRAW);
...
DD_Event DD_ParticleSys::Create(DD_Event& event) {
  if (event.m_type == "generate_emitter") {
    if (m_resBin->emitter_counter >= 50) {
      ...
      DD_Emitter* em = ResSpace::getNewDD_Emitter(m_resBin, vars->ID.c_str());
      em->Initialize(vars->ID.c_str(), vars->parentID.c_str(), vars->radius,
                     vars->lifetime, vars->size, (int)vars->emitPerSec,
                     vars->type, vars->model, vars->deathRate, vars->rotPerSec,
                     vars->direction, vars->textureSet, vars->velUp);
    ...
    if (m_activeClothes >= 20) {
      ...
      m_clothes[m_activeClothes].Initialize(
          vars->ID.c_str(), (size_t)vars->rowSize, (size_t)vars->colSize,
          vars->pointDist, vars->firstPoint, vars->pinnedPoints);
...
bool DD_ParticleSys::Draw(const float dt, glm::mat4 view, glm::mat4 proj,
      ...
      if (em->texSetExists()) {
        ...
        switch (em->tex()) {
          case RenderTextureSets::STAR:
            tex01 = &m_resBin->textures[m_texSetIndex[RenderTextureSets::STAR]];
          ...
          case RenderTextureSets::FIRE01:
            tex01 =
                &m_resBin->textures[m_texSetIndex[RenderTextureSets::FIRE01]];
            m_prend_sh.setUniform("texFlag01", true);
            glActiveTexture(GL_TEXTURE0);
            m_prend_sh.setUniform("ParticleTex01", 0);
            glBindTexture(GL_TEXTURE_2D, tex01->handle);

            tex01 = &m_resBin->textures[m_texSetIndex[RenderTextureSets::SMOG]];
          ...
          case RenderTextureSets::B_BALL:
            tex01 =
                &m_resBin->textures[m_texSetIndex[RenderTextureSets::B_BALL]];
            m_prend_sh.setUniform("texFlag01", true);
            glActiveTexture(GL_TEXTURE0);
            m_prend_sh.setUniform("ParticleTex01", 0);
            glBindTexture(GL_TEXTURE_2D, tex01->handle);
  ...
  for (size_t i = 0; i < m_numJobsA; i++) {
    ...
    DD_Shader* shader =
        ResSpace::findDD_Shader(m_resBin, m_jobsA[i].shader_ID.c_str());
    DD_Texture2D* tex =
        ResSpace::findDD_Texture2D(m_resBin, m_jobsA[i].texture_ID.c_str());
  ...
  for (size_t i = 0; i < m_resBin->water_counter; i++) {
    ...
    DD_Texture2D* n_tex =
        ResSpace::findDD_Texture2D(m_resBin, _w->m_normTex.c_str());
    ...
    if (n_tex) {
      ...
      m_wrend_sh.setUniform("fake_normals", GLboolean(false));
    }
    ...
    DD_Texture2D* tex01 = findAsset_Name<DD_Texture2D*>(m_resBin,
    _w->m_normTex.c_str());
    glActiveTexture(GL_TEXTURE0);
    ...
    glDrawElements(GL_TRIANGLE_STRIP, (GLsizei)_w->m_indices.size(),
                   GL_UNSIGNED_INT, 0);
    glBindVertexArray(0);
  }
  ...
  for (size_t i = 0; i < m_activeClothes; i++) {
    ...
    for (size_t j = 0; j < numIterations; j++) {
      glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 0, cl->m_posBuf[currentBuf]);
      glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1,
                       cl->m_posBuf[1 - currentBuf]);
      glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 2, cl->m_velBuf[currentBuf]);
      glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 3,
                       cl->m_velBuf[1 - currentBuf]);
      glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 4, cl->m_oldBuf);
    ...
    glDrawElements(GL_TRIANGLE_STRIP, (GLsizei)cl->m_indices.size(),
                   GL_UNSIGNED_INT, 0);
    glBindVertexArray(0);
...
int DD_ParticleSys::Generate(DD_Emitter* em, const float dt) {
  ...
  if (remaining > 0) {
    ...
    m_pinitcp_sh.setUniform("createEndIndex", (GLint)em->scratch_numCreated);
...
void DD_ParticleSys::LoadToGPU(DD_Emitter* em) {
  ...
  glBufferData(GL_SHADER_STORAGE_BUFFER,
               (GLuint)em->numParticles() * sizeof(glm::vec4), NULL,
               GL_DYNAMIC_DRAW);
  ...
  glBufferData(GL_SHADER_STORAGE_BUFFER,
               (GLuint)em->numParticles() * sizeof(float), &initData[0],
               GL_DYNAMIC_DRAW);
...
void DD_ParticleSys::LoadToGPU(DD_Cloth* cloth) {
  ...
  glBufferData(GL_SHADER_STORAGE_BUFFER, cloth->m_points.sizeInBytes(),
               &cloth->m_points[0], GL_DYNAMIC_DRAW);

  glGenBuffers(1, &cloth->m_posBuf[1]);
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, cloth->m_posBuf[1]);
  glBufferData(GL_SHADER_STORAGE_BUFFER, cloth->m_points.sizeInBytes(), NULL,
               GL_DYNAMIC_DRAW);

  glGenBuffers(1, &cloth->m_velBuf[0]);
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, cloth->m_velBuf[0]);
  glBufferData(GL_SHADER_STORAGE_BUFFER, cloth->m_velocity.sizeInBytes(),
               &cloth->m_velocity[0], GL_DYNAMIC_DRAW);

  glGenBuffers(1, &cloth->m_velBuf[1]);
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, cloth->m_velBuf[1]);
  glBufferData(GL_SHADER_STORAGE_BUFFER, cloth->m_velocity.sizeInBytes(), NULL,
               GL_DYNAMIC_DRAW);

  glGenBuffers(1, &cloth->m_oldBuf);
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, cloth->m_oldBuf);
  glBufferData(GL_SHADER_STORAGE_BUFFER, cloth->m_oldPos.sizeInBytes(),
               &cloth->m_oldPos[0], GL_DYNAMIC_DRAW);

  glGenBuffers(1, &cloth->m_normBuf);
  glBindBuffer(GL_SHADER_STORAGE_BUFFER, cloth->m_normBuf);
  glBufferData(GL_SHADER_STORAGE_BUFFER, cloth->m_normal.sizeInBytes(),
               &cloth->m_normal[0], GL_DYNAMIC_DRAW);
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, cloth->m_indices.sizeInBytes(),
               &cloth->m_indices[0], GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, cloth->m_uvs.sizeInBytes(), &cloth->m_uvs[0],
               GL_STATIC_DRAW);
