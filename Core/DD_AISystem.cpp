...
DD_AINode generateLOS(DD_AIObject * ai_obj,
					  AI_Agent * agent,
					  const int index,
	...
	if( event.m_type.compare("update_AI") == 0 ) {
		...
		for( size_t i = 0; i < res_ptr->ai_obj_counter; i++ ) {
			...
			if( ai->line_mesh.compare("") != 0 ) {
				DD_LineAgent* _l = ResSpace::findDD_LineAgent(
					res_ptr, ai->line_mesh.c_str());
				if( _l ) { _l->flag_render = ai->flag_render; }
			}
		}
...
//#pragma omp for
		...
		for( size_t i = 0; i < res_ptr->ai_agent_counter; i++ ) {
			...
			if( agent->move_now ) SimulateCrowd(
				&res_ptr->ai_agents[i], event.m_time
			);
		}
...
/**
	Verifies if a node is a duplicate and if not, adds it to the frontier (in
...
/**
	Expands unopened node, provided by index, and adds to priority queue. Checks
	for duplicates
...
/**
	...
	\param agent AI_Agent object containing individual AI agent data
	\param index Location of node in container having PRM connections created
...
/**
	Places goal node in PRM by linking PRM nodes to goal node. Number of links
...
void ConnectToGoalNode(DD_AINode& goal, AStarData& data)
{
	for( size_t i = 0; i < goal.costs.size(); i++ ) {
		...
		_node.connections[new_size - 1].pos02 = glm::vec4(
			data.points[goal.index], 1.f);
	}
}
...
void DD_AISystem::generatePRM(DD_AIObject * ai_obj)
{
	...
	if( ai_obj->new_nodes ) {
		...
		if( ai_obj->node_mesh.compare("") != 0 ) {
			DD_Agent* bulb = ResSpace::findDD_Agent(
				res_ptr, ai_obj->node_mesh.c_str());
			if( bulb ) {
...
void DD_AISystem::mapPRMtoLines(DD_AIObject * ai_obj)
{
	if( ai_obj->line_mesh.compare("") != 0 ) {
		DD_LineAgent* l_a = ResSpace::findDD_LineAgent(
			res_ptr, ai_obj->line_mesh.c_str());
		if( !l_a ) {
...
void DD_AISystem::createLinePath(AI_Agent * agent, const char * lineID)
{
	...
	for( size_t i = 0; i < numLines; i++ ) {
		if( i == 0 ) {
			glm::vec4 a = glm::vec4(
				agent->current_pos.x, 5.f, agent->current_pos.z, 0.f);
			glm::vec4 b = glm::vec4(
				agent->path[0].x, 5.f, agent->path[0].z, 0.f);
			_l->lines[0].pos01 = a;
			_l->lines[0].pos02 = b;
		}
		else {
			glm::vec4 a = glm::vec4(
				agent->path[i - 1].x, 5.f, agent->path[i - 1].z, 0.f);
			glm::vec4 b = glm::vec4(
				agent->path[i].x, 5.f, agent->path[i].z, 0.f);
			_l->lines[i].pos01 = a;
			_l->lines[i].pos02 = b;
		}
	}
...
*/
void DD_AISystem::moveAlongPath(DD_AIObject* ai_obj,
								AI_Agent * agent,
								const float dt)
{
	if( agent->path.size() == 1 ) {
		...
		for( int i = index; i < (int)agent->path.size() && no_intersect; i++ ) {
			...
			for( size_t j = 0; j < num_obstacles && no_intersect; j++ ) {
				...
				p.y = 0.f;
				const float r = ai_obj->obstacles[j].radius +
					ai_obj->agent_radius;
				if( DD_MathLib::checkRayCircle(ray, glm::vec3(pos.x, 0.f, pos.z), p, r) ) {
					no_intersect = false;
					index = i - 1;
				}
				else { index = i; }
		...
		// plot course
		agent->path_index = (index < (int)agent->path_index) ?
			agent->path_index : index;
...
float DD_AISystem::timeToCollision(AI_Agent * agentA, AI_Agent * agentB)
{
	...
	if( c < 0 ) { return 0.f; } // agents are colliding

	glm::vec3 v = agentA->curr_vel - agentB->curr_vel;
	float a = glm::dot(v, v);
	float b = glm::dot(w, v);
	float discrim = b * b - a * c;
	if( discrim <= 0 ) { return std::numeric_limits<float>::infinity(); }

	float tau = (b - sqrtf(discrim)) / a;
	if( tau < 0 ) { return std::numeric_limits<float>::infinity(); }
...
void DD_AISystem::SimulateCrowd(AI_Agent * agent, const float dt)
{
	...
	for( size_t i = 0; i < bin.size(); i++ ) {
		...
		if( other->m_ID == agent->m_ID ) { continue; } // skip myself

		float dist = glm::distance(other->current_pos, agent->current_pos);
		if( dist > agent->sense_n_horizon[0] ) { continue; } // skip far away agents
