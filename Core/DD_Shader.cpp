...
void DD_Shader::QueryShaderAttributes()
{
	...
	GLint numAttribs;
	glGetProgramInterfaceiv(m_Program, GL_PROGRAM_INPUT,
	...
	for( int i = 0; i < numAttribs; ++i ) {
		GLint results[3];
		glGetProgramResourceiv(m_Program, GL_PROGRAM_INPUT, i, 3,
							   properties, 3, NULL, results);

		GLint nameBufSize = results[0] + 1;
		char* name = new char[nameBufSize];
		glGetProgramResourceName(m_Program, GL_PROGRAM_INPUT, i,
...
void DD_Shader::QueryUniforms()
{
	...
	for( int i = 0; i < numUniforms; ++i ) {
		GLint results[4];
		glGetProgramResourceiv(m_Program, GL_UNIFORM, i, 4, properties, 4,
							   NULL, results);

		GLint nameBufSize = results[0] + 1;
		char* name = new char[nameBufSize];
		glGetProgramResourceName(m_Program, GL_UNIFORM, i, nameBufSize,
								 NULL, name);
		printf("%-5d %s (%s)\n", results[2], name, getTypeString(results[1]));
		delete[] name;
	}
