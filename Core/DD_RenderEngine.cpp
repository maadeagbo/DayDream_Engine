...
GLuint bboxEBO;
GLfloat bboxBin[8 * 3];
GLuint bboxInd[4 * 3 * 2] = {0, 1, 1, 2, 2, 3, 3, 1, 0, 4, 1, 5,
...
// Renders 1X1 quad
void RendSpace::RenderQuad(DD_Shader* shader, const glm::mat4& identityMatrix) {
  if (quadVAO == 0) {
...
void RendSpace::RenderVR_Split(DD_Shader* shader,
                               const glm::mat4& identityMatrix, const int eye) {
  if (vrRendered == false) {
    GLfloat vrLeft[] = {
        // Positions        // Texture Coords
        -1.0f, 1.0f, 0.0f, 0.0f, 1.0f, -1.0f, -1.0f, 0.0f, 0.0f, 0.0f,
...
void RendSpace::RenderLine(DD_Shader* shader, const glm::mat4& MVP,
                           dd_array<glm::vec4>& bin, const glm::vec4 color) {
  // Initialize
  if (lineVAO == 0) {
    // Setup plane VAO
    ...
    // glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec4) * 4, NULL,
    // GL_DYNAMIC_DRAW);
    glEnableVertexAttribArray(0);
...
void RendSpace::BindPassTexture(DD_Shader* shader, const CubeMapBuffer* cBuf,
...
void RendSpace::BindPassTexture(DD_Shader* shader, const ShadowBuffer* sBuf,
...
void RendSpace::CreateTex(const GLenum texUnit, const GLenum format,
                          GLuint& texID, const int width, const int height) {
...
ShadowBuffer RendSpace::CreateShadowBuffer(const int width, const int height) {
  ...
  if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
    DD_Terminal::post(
        "ERROR::FRAMEBUFFER:: Shadow Framebuffer is not complete!\n");
  }
  glBindFramebuffer(GL_FRAMEBUFFER, 0);
...
ParticleBuffer RendSpace::CreateParticleBuffer(const int width,
                                               const int height) {
...
CubeMapBuffer RendSpace::CreateCubeMapBuffer(const int width, const int height,
  ...
  GLenum drawBuffers[] = {GL_NONE,
                          GL_COLOR_ATTACHMENT0};  // bind cubeMap for render
  glDrawBuffers(2, drawBuffers);
...
FilterBuffer RendSpace::CreateFilterBuffer(const int width, const int height,
                                           ShadowBuffer* sbuf) {
...
void RendSpace::screenShot(const char* sig, const float game_time,
  ...
  SOIL_save_image(img_name.str(), SOIL_SAVE_TYPE_DDS, width, height, 4,
                  &pixel_write_buffer[0]);
}
...
DD_Event DD_Renderer::RenderHandler(DD_Event& event) {
  ...
  if (event.m_type == "render") {
    ...
    if (m_debugOn) {
      ...
      debug_cbuff.format("Run Time: %.3fs (time to render last frame: %.3fs)",
...
void DD_Renderer::LoadRendererEngine(const GLfloat _Width,
  ...
  m_fbuffer =
      RendSpace::CreateFilterBuffer((int)m_Width, (int)m_Height, &m_sbuffer);
  ...
  m_luminValues = dd_array<GLfloat>(m_lumiTexSizes[2 * count - 2] *
                                    m_lumiTexSizes[2 * count - 1] * 4);
  ...
  m_volumeSphere =
      ResSpace::loadModel_DDM(m_resourceBin, "light_sphere", path.str());
  ...
  if (m_flagVR) {
    if (m_active_cam->vr_window) {
      projMat = CamSpace::GetOffAxisProjMatrix(
          m_active_cam, m_active_cam->vr_scr_pos, m_scrHorzDist, m_scrVertDist,
          m_active_cam->vr_eye_pos);
    }
    /**/
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      if (m_flagCubeMap) {
        ...
        glm::mat4 skyView =
            glm::mat4(glm::mat3(viewMat));  // remove translation
      ...
      glBlitFramebuffer(0, 0, (GLint)m_Width, (GLint)m_Height, 0, 0,
                        (GLint)m_Width, (GLint)m_Height, GL_DEPTH_BUFFER_BIT,
                        GL_NEAREST);
    ...
    if (screen_shot_on) {
      ...
      fps60_tick = 0.f;
      RendSpace::screenShot(screen_shot_name.str(), m_time->getTimeFloat(),
...
void DD_Renderer::DrawDynamicCube(float dt) {
  ...
  glm::quat cam_rots[] = {glm::quat(glm::vec3(0.f, glm::radians(-90.f), 0.f)),
  ...
  for (size_t i = 0; i < 6; i++) {
    ...
    if (up_down) {
      ...
      glm::mat4 r_mat = glm::translate(glm::mat4(), glm::vec3(0.5f, 0.5f, 0.f));
      r_mat = glm::rotate(r_mat, glm::radians(180.f), glm::vec3(0.f, 0.f, 1.f));
        ...
        GLfloat scale, glm::vec3 color)
{
        ...
        if (currentFont == nullptr) {
                ...
                *currentFont = TextSpace::InitFontLib("calibri", 0, 36, m_Width,
m_Height);
        }
...
void DD_Renderer::FrustumCull(DD_Agent& obj, FrustumBox& frust,
                              glm::mat4& view) {
...
void DD_Renderer::LevelOfDetail(DD_Agent& obj, const int objInstIndex,
                                glm::mat4& viewMat) {
...
void DD_Renderer::GBufferPass(GLfloat dt, glm::mat4& view, glm::mat4& proj,
                              const bool clip, glm::vec4 c_plane) {
  // gbuffer pass
  glBindFramebuffer(GL_FRAMEBUFFER, m_gbuffer.deferredFBO);
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
  glEnable(GL_DEPTH_TEST);
...
void DD_Renderer::ShadowPass(GLfloat dt, VR_Eye eye) {
  ...
  if (shadowL) {
    ...
    glViewport(0, 0, m_sbuffer.width,
               m_sbuffer.height);  // render shadow dimensions
    ...
    glm::mat4 lightView =
        glm::lookAt(glm::vec3(pos), center, glm::vec3(m_active_cam->worldUp()));
    // use camera's view frustum to calculate ortho boundaries
    glm::vec4 corners[8];
    for (size_t i = 0; i < 8; i++) {
      glm::vec3 temp = m_active_cam->frustum().corners[i];
      corners[i] = lightView * glm::vec4(temp.x, temp.y, temp.z, 1.0f);
    }
    ...
    glm::mat4 lightProjection = glm::ortho(
        glm::floor(min.x), glm::floor(max.x), glm::floor(min.y),
        glm::floor(max.y), m_active_cam->near_plane, m_active_cam->far_plane);
    ...
    for (size_t i = 0; i < m_resourceBin->m_num_agents; i++) {
      ...
      if (agent->flag_render && mdl_exists) {
        shader = (agent->flag_model) ? &m_shaders[Shaders::DEPTH]
                                     : &m_shaders[Shaders::DEPTH_SKINNED];
        if (agent->inst_m4x4.size() > 1) {
          // render with instance buffer
          InstanceSetup(dt, identity, identity, agent, shader, true, shadowL);
        }
        if (agent->inst_m4x4.size() == 1) {
          // render as single instance
          InstanceSetup(dt, identity, identity, agent, shader, false, shadowL);
        }
      }
    }
...
void DD_Renderer::InstanceSetup(float dt, glm::mat4& view, glm::mat4& proj,
                                DD_Agent* agent, DD_Shader* shader,
                                const bool flagInstance, DD_Light* shadow) {
  ...
  if (flagInstance) {
    ...
    for (size_t j = 0; j < numModels; j++) {
      ...
      if (agent->flag_modelsk) {
        ...
        SkinnedMeshRender(shader, agent, j, bufferOffset, proj, view, shadow);
      } else {
        StaticMeshRender(shader, agent, j, bufferOffset, proj, view, shadow);
      }
    }
  } else {
    // render single instance (10 LOD levels supported)
    size_t m_idx =
        (agent->mesh_buffer.size() == 1) ? 0 : agent->flag_cull_inst[0];
    m_LODBufferSize[m_idx] = 1;
    if (agent->flag_modelsk) {
      // render animated mesh
      shader = (shadow) ? shader : &m_shaders[Shaders::SKINNED];
      SkinnedMeshRender(shader, agent, m_idx, bufferOffset, proj, view, shadow);
    } else {
      StaticMeshRender(shader, agent, m_idx, bufferOffset, proj, view, shadow);
    }
  }
}

void DD_Renderer::StaticMeshRender(DD_Shader* shader, DD_Agent* agent,
                                   const size_t modelIndex,
                                   const size_t bufferOffet, glm::mat4& proj,
                                   glm::mat4& view, DD_Light* shadow) {
  ...
  for (size_t i = 0; i < model->meshes.size(); i++) {
    if (!shadow) {
      ...
      mat = (mat_index < 0)
                ...
                : ResSpace::findDD_Material(
                      m_resourceBin, (unsigned)agent->mat_buffer[mat_index]);
      ...
      glm::mat4 norm = glm::transpose(
          glm::inverse(agent->parent_transform * agent->f_inst_m4x4[0]));
    ...
    if (agent->flag_color) {
      ...
      glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(glm::vec3), &defaultColor[0]);
    }
...
void DD_Renderer::SkinnedMeshRender(DD_Shader* shader, DD_Agent* agent,
                                    const size_t modelIndex,
                                    const size_t bufferOffet, glm::mat4& proj,
                                    glm::mat4& view, DD_Light* shadow) {
  ...
  for (size_t i = 0; i < modelsk->meshes.size(); i++) {
    ...
    // glDrawElements(GL_TRIANGLES, meshes[i].indices.size(), GL_UNSIGNED_INT,
    // 0);
    glBindVertexArray(0);
  }
}

void DD_Renderer::LightPass(DD_Shader* shader, DD_Camera* cam, glm::mat4& view,
                            glm::mat4& proj, VR_Eye eye) {
  glm::mat4 identity, vrProj, vrView;
  switch (eye) {
    case LEFT:
      vrView =
          CamSpace::GetViewMatrixVREye(cam, true, false, cam->cam_eye_dist);
      vrProj = CamSpace::GetOffAxisProjMatrix(
          cam, cam->vr_scr_pos, m_scrHorzDist, m_scrVertDist, cam->vr_eye_pos);
      break;
    case RIGHT:
      vrView =
          CamSpace::GetViewMatrixVREye(cam, false, true, cam->cam_eye_dist);
      vrProj = CamSpace::GetOffAxisProjMatrix(
          cam, cam->vr_scr_pos, m_scrHorzDist, m_scrVertDist, cam->vr_eye_pos);
      break;
    case DEFAULT:
  ...
  for (size_t i = 0; i < m_resourceBin->light_counter; i++) {
    ...
    glm::vec3 lightPos = glm::vec3(lghtInfo->parent_transform *
                                   glm::vec4(lghtInfo->_position, 1.0f));
    ...
    if (lghtInfo->m_type == LightType::DIRECTION_L) {
      ...
      GLfloat camDist =
          glm::distance(glm::vec3(cam->pos()), lghtInfo->_position);
      ...
      if (camDist < lvRadius * 1.05f) {
        ...
        // stencil test to be enabled to succeed always. Only the depth test
        // matters.
...
void DD_Renderer::RenderLightSphere(DD_Shader* shader, const glm::mat4 MVP) {
  ...
  glDrawElements(GL_TRIANGLES, (GLint)m_volumeSphere->meshes[0].indices.size(),
                 GL_UNSIGNED_INT, 0);
  glBindVertexArray(0);
...
GLfloat DD_Renderer::ComputeLuminance() {
  ...
  glBindImageTexture(0, m_lbuffer.colorTex, 0, GL_FALSE, 0, GL_READ_ONLY,
                     GL_RGBA16F);
  glBindImageTexture(1, m_luminOutput[0], 0, GL_FALSE, 0, GL_WRITE_ONLY,
                     GL_RGBA16F);
  ...
  for (int i = 1; i < count; ++i) {
    glBindImageTexture(0, m_luminOutput[i - 1], 0, GL_FALSE, 0, GL_READ_ONLY,
                       GL_RGBA16F);
    glBindImageTexture(1, m_luminOutput[i], 0, GL_FALSE, 0, GL_WRITE_ONLY,
                       GL_RGBA16F);
  ...
  avgLumCol /= numPixels;
  // printf("red: %.3f, green: %.3f, blue: %.3f\r", avgLumCol.r, avgLumCol.g,
  // avgLumCol.b);
...
void SortSpace::QuickSort(DD_Agent* agent, const int p, const int r,
