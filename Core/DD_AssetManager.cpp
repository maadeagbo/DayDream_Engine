...
bool add_rigid_body(DD_BaseAgent *agent, DD_MeshData *mdata);
...
int dd_assets_create_cam(lua_State *L) {
  ...
  DD_Cam *new_cam = nullptr;
  // get arguments
  const char *id = fb.get_func_val<const char>("id");
  if (id) {
    new_cam = spawnDD_Cam(getCharHash(id));
...
int dd_assets_create_light(lua_State *L) {
  ...
  DD_Bulb *new_bulb = nullptr;
  // get arguments
  const char *id = fb.get_func_val<const char>("id");
  if (id) {
    new_bulb = spawnDD_Bulb(getCharHash(id));
