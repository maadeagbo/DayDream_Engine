...
void DD_Emitter::Initialize(const char* ID, const char* parent,
                            const float radius, const float lifetime,
                            const float size, const int _emitPerSec,
                            const EmitterType type, glm::mat4& placement,
                            const float deathR, const float rotationPS,
                            glm::mat4& _direc, const RenderTextureSets ts,
                            const float streamF) {
...
float myRand(const float a, const float b) {
  return a +
         static_cast<float>(rand()) / (static_cast<float>(RAND_MAX / (b - a)));
}

void DD_Cloth::Initialize(const char* ID, const size_t rowSize,
                          const size_t colSize, const float pointDist,
                          const glm::vec3 firstPos, dd_array<size_t> pinned) {
  ...
  m_indices.resize((colSize * 2) * (rowSize - 1) +
                   (rowSize - 1));  // triangle strip
  ...
  for (size_t i = 0; i < rowSize; i++) {    // row
    for (size_t j = 0; j < colSize; j++) {  // column
      ...
      m_points[index] = glm::vec4(0.f, 0.f, 0.f, 1.f) +
                        glm::vec4(j * pointDist, i * -pointDist, 0.f, 1.f);
      m_points[index] = transf * m_points[index];
      m_oldPos[index] = m_points[index];

      m_velocity[index] = glm::vec4(0.0);
      m_uvs[index] =
          glm::vec2((float)j / (float)colSize, (float)i / (float)rowSize);
    }
  }
  // generate EBO
  size_t currIndex = 0;
  for (size_t i = 0; i < rowSize - 1; i++) {
    ...
    m_indices[currIndex] = PRIM_RESTART;
    currIndex += 1;
  }
}
...
void DD_Water::initialize(const size_t rowSize, const size_t colSize,
                          const float x_pointDist, const float y_pointDist,
                          const glm::vec3 firstPos, const size_t tex_repeat) {
  glEnable(GL_PRIMITIVE_RESTART);
  glPrimitiveRestartIndex(PRIM_RESTART);
  active = true;
  ...
  glBufferData(GL_SHADER_STORAGE_BUFFER, m_pos.sizeInBytes(), &m_pos[0][0],
               GL_DYNAMIC_DRAW);
  ...
  for (size_t i = 0; i < 3; i++) {
    ...
    glBufferData(GL_SHADER_STORAGE_BUFFER, m_vel.sizeInBytes(), &m_pos[0][0],
                 GL_DYNAMIC_DRAW);
  }
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_indices.sizeInBytes(), &m_indices[0],
               GL_STATIC_DRAW);
  ...
  glBufferData(GL_ARRAY_BUFFER, m_uvs.sizeInBytes(), &m_uvs[0], GL_STATIC_DRAW);
...
#pragma omp parallel for
  for (int i = 0; i < (int)m_rowS - 1; i++) {
    for (int j = 0; j < (int)m_colS; j++) {
      ...
      // main momentum in x
      m_xhalf[i][j].x = (vx_00 + vx_01) * 0.5f -
                        (dt * 0.5f) *
                            (sq_f(vx_01) / h_01 + hgrav * sq_f(h_01) -
                             sq_f(vx_00) / h_00 - hgrav * sq_f(h_00)) /
                            m_xDist;
      // height in y
      m_xhalf[i][j].y =
          (h_00 + h_01) * 0.5f - (dt * 0.5f) * (vx_01 - vx_00) / m_xDist;
      // momentum in z
      m_xhalf[i][j].z = (vz_00 + vz_01) * 0.5f -
                        (dt * 0.5f) *
                            ((vx_01 * vz_01) / h_01 - (vx_00 * vz_00) / h_00) /
                            m_xDist;
...
#pragma omp parallel for
  for (int i = 0; i < (int)m_rowS; i++) {
    for (int j = 0; j < (int)m_colS - 1; j++) {
      ...
      // main momentum in z
      m_zhalf[i][j].z = (vz_00 + vz_01) * 0.5f -
                        (dt * 0.5f) *
                            (sq_f(vz_01) / h_01 + hgrav * sq_f(h_01) -
                             sq_f(vz_00) / h_00 - hgrav * sq_f(h_00)) /
                            m_yDist;
      // height in y
      m_zhalf[i][j].y =
          (h_00 + h_01) * 0.5f - (dt * 0.5f) * (vz_01 + vz_00) / m_yDist;
      // momentum in x
      m_zhalf[i][j].x = (vx_01 + vx_00) * 0.5f -
                        (dt * 0.5f) *
                            ((vz_01 * vx_01) / h_01 - (vz_00 * vx_00) / h_00) /
                            m_yDist;
  ...
  for (int i = 1; i < (int)m_rowS - 1; i++) {
    for (int j = 0; j < (int)m_colS; j++) {
      ...
      m_vel[i][j].x -= dt_x * (dampc * m_vel[i][j].x +
                               sq_f(m_xhalf[i][j].x) / m_xhalf[i][j].y +
                               hgrav * sq_f(m_xhalf[i][j].y) -
                               sq_f(m_xhalf[i - 1][j].x) / m_xhalf[i - 1][j].y -
                               hgrav * sq_f(m_xhalf[i - 1][j].y));
    }
  }
...
#pragma omp parallel for
  for (int i = 0; i < (int)m_rowS; i++) {
    for (int j = 0; j < (int)m_colS; j++) {
      glm::vec4 point_v4 =
          m_transpose_mat * glm::vec4(i * m_xDist, j * m_yDist, 0.0, 0.0);
      const glm::vec2 point = glm::vec2(point_v4);
      const float dist = glm::distance(point, pos_xz);

      if (dist < radius) {
        m_pos[i][j].y += height * (1 - dist / radius);
      }
    }
  }
}
