...
namespace DD_MathLib {
	...
	bool checkRayCircle(const glm::vec3 ray,
	...
	raycastBuff rayCast(const int mouseX,
						const int mouseY,
						const char* agentID = "");
	glm::vec3 rayCastW(const int mouseX, const int mouseY);
	void randomSampleRect(const glm::vec3 center,
						  glm::vec3 min, glm::vec3 max,
						  dd_array<glm::vec3>& points);
	size_t calculateLOSLines(const size_t index,
