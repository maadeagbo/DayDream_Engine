...
namespace PsuedoUnitTest {
	namespace ArrayTests {
		...
		void Print2DArray_int(	dd_2Darray<int>& dd_container, 
								const char* name = "") {
		...
		void SimpleTest() {
			...
			printf("\tTime to fill in test_one and test_two arrays: %" PRIu64 
				" ns\n", end_t - start_t
			);
	...
	namespace OBJParserTests {
		void SimpleTests() {
			...
			DD_Model* testmodel = 
				std::move(ResSpace::loadModel(objLoc2.c_str()));
		...
		void SimpleTests() {
			...
			for (size_t i = 0; i < res.tex_counter; i++) {
				...
				printf("\tDimensions: width %d height %d\n",
					res.textures[i]->Width, res.textures[i]->Height);
			}
	...
	namespace RendererTest {
		void SimpleTests(float width, float height, SDL_Window* win) {
			...
			DD_Resources res = DD_Resources();
			res = std::move(ResSpace::Load(
				(RESOURCE_DIR + "sample01").c_str())
			);
			DD_Queue sampleQ = DD_Queue(5);
			res.queue = &sampleQ;
			...
			EventHandler handle = std::bind(&DD_Renderer::RenderHandler,
				&renderer, std::placeholders::_1);
