...
TYPE* getNew##TYPE(DD_Resources* res, const char* id) \
{ \
	if(res->ITER >= res->BNAME.size()) { return nullptr; } \
...
namespace ResSpace {
	...
	void loadAgent_ID(DD_Resources* res,
					  const char* agentID,
					  bool mem_flag = false);

	void updateSceneGraph(DD_Resources* res, const float dt);

	void deleteAgent(DD_Resources* res, 
					 const char* agent_id, 
