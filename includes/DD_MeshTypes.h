...
*	DD_LineAgent:
*		- Container for all Line rendering elements (uses LinePoint && XZLine)
*	Bounding Box:
*		-
*
*	TODO:
...
struct obj_vec3
{
	...
	{
		return obj_vec3(data[0] + other.data[0],
						data[1] + other.data[1],
						data[2] + other.data[2]
		);
	}
	...
	{
		float mag = std::sqrt(data[0] * data[0] + data[1] * data[1] +
							  data[2] * data[2]);
...
struct obj_mat
{
	...
	obj_vec3 diffuseRaw;
	std::string albedo_tex, specular_tex, metalness_tex, roughness_tex, normal_tex,
		emissive_tex, ao_tex;
	bool albedo_flag, spec_flag, metal_flag, rough_flag, norm_flag,
		emit_flag, ao_flag, multiplier;
