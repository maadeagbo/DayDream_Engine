...
public:
	...
	void Initialize(const char* ID, const char* parentID, const float radius,
					...
					const EmitterType type, glm::mat4& placement, const float deathR,
					const float rotationPS, glm::mat4& _direc, const RenderTextureSets ts,
					const float streamF);
	bool isDead();
...
struct DD_Cloth
{
	...
	void Initialize(const char* ID, const size_t rowSize, const size_t colSize,
					const float pointDist, const glm::vec3 firstPos, dd_array<size_t> pinned);
...
struct DD_Water
{
	...
	GLuint m_VAO, m_EBO, m_posBuf[3], m_velBuf[3], m_uvBuf, m_normBuf,
		m_rowS, m_colS;
