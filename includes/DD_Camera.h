...
{
public:
	DD_Camera(const GLfloat screenW = 0.0f,
			  const GLfloat screenH = 0.0f,
			  const GLfloat fovh = 45.0f,
			  const GLfloat nearP = 1.0f,
...
namespace CamSpace {
	...
	glm::mat4 GetViewMatrixVREye(const DD_Camera* cam,
								 const bool _left,
								 const bool _right,
								 const float dist);
	glm::mat4 GetPerspecProjMatrix(const DD_Camera* cam);
	glm::mat4 GetOffAxisProjMatrix(
		const DD_Camera* cam,
