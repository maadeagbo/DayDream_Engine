...
*
*	DD_Renderer:
*		- manages deferred renderer and renders GameObjects, Lights,
etc...
...
enum VR_Eye { LEFT, RIGHT, DEFAULT };
...
struct DD_Renderer {
  ...
  void DrawTextToScreen(std::string text, GLfloat x, GLfloat y, GLfloat scale,
                        glm::vec3 color);
  ...
  void GBufferPass(GLfloat dt, glm::mat4& view, glm::mat4& proj,
                   const bool clip = false, glm::vec4 c_plane = glm::vec4());
  void ShadowPass(GLfloat dt, VR_Eye eye = VR_Eye::DEFAULT);
  void InstanceSetup(float dt, glm::mat4& view, glm::mat4& proj,
                     DD_Agent* agent, DD_Shader* shader,
                     const bool flagInstance = false,
                     DD_Light* shadow = nullptr);
  void StaticMeshRender(DD_Shader* shader, DD_Agent* agent,
                        const size_t modelIndex, const size_t bufferOffet,
                        glm::mat4& proj, glm::mat4& view,
                        DD_Light* shadow = nullptr);
  void SkinnedMeshRender(DD_Shader* shader, DD_Agent* agent,
                         const size_t modelIndex, const size_t bufferOffet,
                         glm::mat4& proj, glm::mat4& view,
                         DD_Light* shadow = nullptr);
  void LightPass(DD_Shader* shader, DD_Camera* cam, glm::mat4& view,
                 glm::mat4& proj, VR_Eye eye = VR_Eye::DEFAULT);
...
void RenderLine(DD_Shader* shader, const glm::mat4& MVP,
...
CubeMapBuffer CreateCubeMapBuffer(const int width, const int height,
                                  DD_Skybox* sb);
FilterBuffer CreateFilterBuffer(const int width, const int height,
                                ShadowBuffer* sbuf);
void screenShot(const char* sig, const float game_time, const unsigned width,
