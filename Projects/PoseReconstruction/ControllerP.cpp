...
namespace {
	float speed = 500.0f, mouseSensitivity = 0.5f, Pitch = 0.f, Yaw = 0.f;
	float lastClick = 0.f;
	glm::quat OriginalRot;
}

ControllerP::ControllerP(const char * ID, const char * model, const char * parent) :
	clickedP(false), clickedT(false), clickedSpace(false), clickedRA(false),
	clickedLA(false), ignore_controls(false)
{
	m_ID = ID;
	// store function pointer
	EventHandler update =
		std::bind(&ControllerP::Update, this, std::placeholders::_1);
...
DD_Event ControllerP::Update(DD_Event event) {
...
void ControllerP::initRot(const float rad_pitch, const float rad_Yaw) {
	Pitch = rad_pitch;
	Yaw = rad_Yaw;
}
